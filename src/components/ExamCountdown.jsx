/**
 * ExamCountdown — shows upcoming exams/assignments with a manual entry form.
 */
...
function CountdownCard({ event, onDelete }) {
  ...
  return (
    <div
      className={`
        group relative flex-shrink-0 w-[200px] sm:w-[220px] rounded-2xl border
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="bg-[#0d0d1a] border border-white/10 rounded-2xl p-6 space-y-5 w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between">
          <p className="font-bold text-white text-base">Add Exam / Assignment</p>
...
export default function ExamCountdown() {
  ...
  function handleAdd(eventData) {
    const updated = addManualEvent(eventData);
    setEvents(updated);
  }

  function handleDelete(id) {
    const updated = deleteManualEvent(id);
    setEvents(updated);
  }
    ...
    <div className="space-y-3">
      ...
      {events.length === 0 ? (
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none">
          ...
          <button
            onClick={() => setShowModal(true)}
            className="flex-shrink-0 w-[120px] rounded-2xl border border-dashed border-white/10 hover:border-white/20 hover:bg-white/[0.02] flex flex-col items-center justify-center gap-2 transition-all"
          >
            <Plus className="w-5 h-5 text-white/20" />
