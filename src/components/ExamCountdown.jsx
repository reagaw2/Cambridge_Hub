...
import {
  ...
  markEventComplete, unmarkEventComplete,
...
function EventCard({ event, cfg, onComplete, onDelete, onEdit }) {
  ...
  const countNum = isToday ? null : isPast ? null : days;
  ...
  return (
    <div className={`group relative flex-shrink-0 w-36 rounded-xl border ${cfg.cardBorder} bg-white/[0.04] hover:bg-white/[0.07] p-3.5 flex flex-col gap-2 transition-all`}
      style={{ minWidth: 140 }}>

      {/* Hover actions */}
      <div className="absolute top-2 right-2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onEdit(event)} className="p-1 rounded-lg hover:bg-white/15 transition-all">
          <Pencil className="w-2.5 h-2.5 text-white/40" />
        </button>
        <button onClick={() => onDelete(event.id)} className="p-1 rounded-lg hover:bg-red-500/20 transition-all">
      ...
      <div className="flex items-baseline gap-1">
        {countNum !== null ? (
          <>
            <span className={`text-3xl font-black tabular-nums leading-none ${countColor}`}>{countNum}</span>
            <span className="text-xs font-semibold text-white/40 leading-none">days</span>
          </>
        ) : (
      ...
      <div className="space-y-0.5 mt-auto">
        <div className="flex items-center gap-1 text-[10px] text-white/35">
          <span>📅</span>
          <span>{fmtDate(event.due_date)}</span>
        </div>
        <div className="flex items-center gap-1 text-[10px] text-white/35">
          <span>🕐</span>
          <span>{fmtTime(event.due_date)}</span>
        </div>
      </div>
...
function AddCard({ cfg, onAdd }) {
  return (
    <button onClick={onAdd}
      className={`flex-shrink-0 w-14 rounded-xl border border-dashed ${cfg.cardBorder} hover:bg-white/[0.04] flex items-center justify-center transition-all`}
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="flex items-center gap-2.5">
        ...
        {upcoming.length > 0 && (
          <span className="font-mono text-[10px] text-white/30">{upcoming.length}</span>
        )}
      </div>
      ...
      <div className="flex gap-2.5 overflow-x-auto scrollbar-none pb-1">
        {upcoming.length === 0 ? (
          <button onClick={onAdd}
            className="flex items-center gap-2 text-xs text-white/25 hover:text-white/50 transition-colors py-1.5">
          ...
          <>
            {upcoming.map(e => (
              <EventCard key={e.id} event={e} cfg={cfg}
...
// ── Completed Section (hidden behind toggle) ───────────────────────────────────
  ...
  return (
    <div className="space-y-2">
      <button onClick={() => setOpen(o => !o)}
        ...
        <span>{open ? "▲" : "▼"}</span>
        <span>{events.length} completed</span>
      </button>
      {open && (
            ...
            return (
              <div key={e.id} className="group flex items-center gap-3 py-2 px-3 rounded-xl border border-white/5 bg-white/[0.015] opacity-50 hover:opacity-75 transition-all">
                <div className={`w-4 h-4 rounded ${cfg.iconBg} flex items-center justify-center shrink-0`}>
                  <span className="text-[7px] font-black text-white">{cfg.iconChar}</span>
                </div>
                ...
                <button onClick={() => onUncomplete(e.id)}
                  className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-white/10 transition-all">
                  ↩
                </button>
                <button onClick={() => onDelete(e.id)}
                  className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-500/15 text-white/30 hover:text-red-400 transition-all">
                  ×
                </button>
              </div>
            );
...
export default function ExamCountdown() {
  // Read from localStorage immediately — no flash
  const [events, setEvents] = useState(() => {
    try { return JSON.parse(localStorage.getItem("exam_countdown_events_v3") ?? "[]"); } catch { return []; }
  });
  const [syncStatus, setSyncStatus] = useState(null);
  const [modal, setModal] = useState(null); // null | { defaultType?, editingEvent? }

  // Background sync — updates quietly without hiding cards
  useEffect(() => {
  ...
  function getEventsForType(type) {
    return sortedUpcoming.filter(e => (e.type ?? "Assignment") === type);
  }
    ...
    <div className="space-y-5">
      ...
      {modal !== null && (
        <EventModal
          ...
          onSave={handleSave}
          editingEvent={modal.editingEvent ?? null}
          defaultType={modal.defaultType ?? null}
        />
      )}
    </div>
  );
