 ...
 */
export default function PaperPdfButton({ label = "Question Paper PDF" }) {
  const { user } = useAuth();
  ...
  async function handleFileChange(e) {
    ...
    setUploading(true);
    setUploadError(null);
    const { url: newUrl, error } = await uploadPaperPdf(file);
    setUploading(false);
    if (error) {
        ...
        {showUpload && (
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-foreground">Upload 9702/12/F/M/25 PDF</p>
