  ...
  if (!open) {
    return (
      <button
        ...
        className={`w-full flex items-center gap-2.5 px-4 py-2.5 rounded-xl border text-xs font-medium transition-all text-left ${
          existing?.text
            ? "bg-amber-500/8 border-amber-500/20 text-amber-300/80 hover:bg-amber-500/12"
            : "bg-secondary/40 border-border/40 text-muted-foreground/50 hover:text-foreground/70 hover:border-border hover:bg-secondary/60"
        }`}
      >
        ...
        {existing?.text && <Pencil className="w-3 h-3 shrink-0 opacity-50" />}
      </button>
    );
  }
    ...
    <div className="bg-amber-500/6 border border-amber-500/20 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-amber-500/15">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[11px] font-bold uppercase tracking-widest text-amber-400/70">My Note</p>
        </div>
