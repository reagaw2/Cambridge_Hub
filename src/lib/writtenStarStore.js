...
function key(subject) {
  return `written_starred_${subject ?? "physics"}_v1_${getSessionUserId()}`;
}
...
async function getStudentRow() {
  ...
  const { data: rows } = await supabaseClient
    .from("StudentData").select("id, written_stars").eq("user_id", user.id);
  return rows?.[0] ?? null;
}
...
export function starQuestion(questionId, { topic = "", questionText = "", markScheme = "", feedback = null, answer = "" } = {}, subject = "physics") {
  ...
  all[questionId] = {
    questionId, topic, questionText, markScheme, feedback, answer, subject,
...
export function saveTeacherQuestion(questionId, teacherQuestion, subject = "physics") {
  const all = readAll(subject);
  if (all[questionId]) { all[questionId] = { ...all[questionId], teacherQuestion }; writeLocal(subject, all); pushToSupabase(subject, all).catch(() => {}); }
  return all;
}

export function saveTeacherResponse(questionId, teacherResponse, subject = "physics") {
  const all = readAll(subject);
  if (all[questionId]) { all[questionId] = { ...all[questionId], teacherResponse }; writeLocal(subject, all); pushToSupabase(subject, all).catch(() => {}); }
  return all;
}
