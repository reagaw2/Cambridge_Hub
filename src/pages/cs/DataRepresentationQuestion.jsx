...
const TOPIC_KEY = "data_representation";
const SUBJECT = "cs";

export default function DataRepresentationQuestion() {
  const navigate = useNavigate();
  const { questions, loading, error, getCurrentQuestion, advance, getProgress } = useSupabaseQuestions(TOPIC_KEY, SUBJECT);
  ...
  if (!loading && questions.length === 0) {
    ...
    const question = override ?? queued.question;
    const idx = override ? 0 : queued.idx;
    const total = override ? 1 : queued.total;
    return (
      <CSQuestionAttempt
  ...
  async function handleSubmit() {
    ...
    const topicKey = question.topic_key ?? TOPIC_KEY;
    ...
    navigate("/cs/feedback", {
      state: {
        ...
        topicRoute: "/cs/data-representation/question",
        backRoute: "/cs/data-representation/question",
        dashRoute: "/cs",
