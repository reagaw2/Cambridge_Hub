...
export default function NuclearQuestionAttempt() {
  ...
  const queued = getNextNuclearQuestion();
  ...
  const handleSubmit = async () => {
    ...
    advanceNuclearIndex();
    navigate("/feedback", { state: { feedback: fb, answer, topicKey: question.topic_key, questionId: question.id, nextFullRoute: question.nextFullRoute ?? "/nuclear/question", nextRetryRoute: question.nextRetryRoute ?? "/nuclear/question", backRoute: "/physics", paperRef: question.paper_ref } });
  };
      ...
      <div className="w-full max-w-[480px] flex flex-col min-h-screen">
        ...
        <div className="flex-1 flex flex-col gap-4 p-4">
          <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="flex justify-end"><span className="font-mono text-xs text-muted-foreground">[{question.total_marks} mark{question.total_marks !== 1 ? "s" : ""}]</span></div>
          </div>
          {showTeachMe ? (
            <TeachMeHow question={question} onFinalSubmit={async (fb, finalAnswer) => { const m = fb.marks_earned ?? 0; await recordAttempt(question.topic_key, m, { total_marks: question.total_marks, question_id: question.id }); writeMistakeDna(fb, question.id, question.topic, m, question.total_marks, finalAnswer).catch(() => {}); await addToReviewBank({ question_id: question.id, topic: question.topic, question_text: question.text, mark_scheme: question.mark_scheme ?? "", total_marks: question.total_marks, first_attempt_score: m, first_attempt_feedback: fb.cambridge_insight ?? "", first_attempt_answer: finalAnswer }); advanceNuclearIndex(); navigate("/feedback", { state: { feedback: fb, answer: finalAnswer, topicKey: question.topic_key, questionId: question.id, nextFullRoute: question.nextFullRoute ?? "/nuclear/question", nextRetryRoute: question.nextRetryRoute ?? "/nuclear/question", backRoute: "/physics", paperRef: question.paper_ref } }); }} onClose={() => setShowTeachMe(false)} />
          ) : (
            <>
          ...
          <DevQuestionJumper allQuestions={NUCLEAR_QUESTIONS} onJump={(q) => { setOverrideQuestion(q); setAnswer(""); setShowTeachMe(false); setError(null); }} />
        </div>
      </div>
